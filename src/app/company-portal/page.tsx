...
export default function CompanyPortalLoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      // In a real app, you would first validate the password here.
      const otpResult = await sendOtp({ email });

      if (otpResult.success) {
        toast({ title: 'Verification Code Sent', description: 'Check your email for the 6-digit code.' });
        router.push(`/login/otp?portal=company&email=${encodeURIComponent(email)}`);
      } else {
      ...
      <form onSubmit={handleSignIn}>
        <Card>
          <CardHeader className="text-center">
            <CardTitle className="font-headline text-2xl">Company Portal</CardTitle>
            <CardDescription>
              Sign in to manage your products and Digital Product Passports.
            </CardDescription>
          </CardHeader>
