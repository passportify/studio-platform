...
export default function VerifierPortalLoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (otpResult.success) {
        ...
        router.push(`/login/otp?portal=verifier&email=${encodeURIComponent(email)}`);
      } else {
      ...
      <form onSubmit={handleSignIn}>
        <Card>
          <CardHeader className="text-center">
            <CardTitle className="font-headline text-2xl">Verifier Portal</CardTitle>
            <CardDescription>
              Sign in to review and attest to compliance claims.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" placeholder="auditor@sgs.com" required value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div className="space-y-2">
          ...
          <CardFooter className="flex flex-col gap-4">
            ...
            <div className="text-xs text-center text-muted-foreground space-x-2">
              ...
              <span>•</span>
              <Link href="/supplier-portal" className="underline hover:text-primary">Material Supplier?</Link>
            </div>
          </CardFooter>
        </Card>
      </form>
