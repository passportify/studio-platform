  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (otpResult.success) {
        ...
        router.push(`/login/otp?portal=superadmin&email=${encodeURIComponent(email)}`);
      } else {
      ...
      <form onSubmit={handleSignIn}>
        <Card>
          <CardHeader className="text-center">
            <CardTitle className="font-headline text-2xl">Super Admin Portal</CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="email" type="email" placeholder="admin@passportify.online" required value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div className="space-y-2">
          ...
          <CardFooter className="flex flex-col gap-4">
            ...
            <div className="text-xs text-center text-muted-foreground">
              <Link href="/" className="underline hover:text-primary">Return to Portal Selection</Link>
            </div>
          </CardFooter>
        </Card>
      </form>
