...
import { mockSignatures, mockEntityNames, mockUserNames } from "@/lib/mock-data";
...
import { Eye, FileCheck, Handshake, MoreHorizontal, FileText, CheckCircle, XCircle, AlertCircle } from "lucide-react";
import { Tooltip, TooltipProvider, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";


export function DigitalSignatureManagementClient() {
  ...
  const handleRevoke = (sigId: string) => {
    setSignatures(prev => prev.map(s => s.signature_id === sigId ? { ...s, signature_status: 'REVOKED' } : s));
    toast({ title: "Signature Revoked" });
  };
  ...
  return (
    <TooltipProvider>
      <Card>
        <CardHeader>
          <CardTitle className="font-headline">Signature Log</CardTitle>
          <CardDescription>A chronological log of all cryptographic signatures created on the platform.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                        ...
                        <TableCell className="text-right">
                           <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    <DropdownMenuItem onClick={() => handleViewDetails(sig)}><Eye className="mr-2 h-4 w-4" /> View Details</DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => handleRevoke(sig.signature_id)} className="text-destructive" disabled={sig.signature_status !== 'SIGNED'}>
                                        <XCircle className="mr-2 h-4 w-4" /> Revoke Signature
                                    </DropdownMenuItem>
                                </DropdownMenuContent>
                            </DropdownMenu>
                        </TableCell>
