...
import { mockAuditLogs, mockChangeLogs, mockEntityNames } from "@/lib/mock-data";
...
import { Eye, Bot, User, Globe, QrCode, UploadCloud, Check, X, FileScan, FilePlus2, Pencil, ArrowRight } from "lucide-react";
...
export function AuditTrailClient() {
  ...
  const [clientAuditLogs, setClientAuditLogs] = useState<FormattedAuditLog[]>([]);
  const [clientChangeLogs, setClientChangeLogs] = useState<FormattedChangeLog[]>([]);
  ...
  const eventConfig: Record<AuditLog['event_type'], { icon: React.ElementType, color: string, badge: "default" | "secondary" | "destructive" }> = {
    Create: { icon: FilePlus2, color: 'text-blue-500', badge: "default" },
    Update: { icon: Pencil, color: 'text-yellow-500', badge: "secondary" },
    Upload: { icon: UploadCloud, color: 'text-sky-500', badge: "secondary" },
    Validation: { icon: FileScan, color: 'text-cyan-500', badge: "secondary" },
    AI_Processing: { icon: Bot, color: 'text-purple-500', badge: "secondary" },
    Approval: { icon: Check, color: 'text-green-500', badge: "default" },
    Rejection: { icon: X, color: 'text-red-500', badge: "destructive" },
    View: { icon: Eye, color: 'text-gray-500', badge: "secondary" },
    QR_Scan: { icon: QrCode, color: 'text-indigo-500', badge: "secondary" },
  };
  ...
  const getEventContextSummary = (log: AuditLog): string => {
    switch (log.event_type) {
      case 'Upload': return `File: ${log.event_context.file_name}`;
      case 'QR_Scan': return `Scanned via ${log.event_channel}`;
      case 'Approval': return `Approved: ${mockEntityNames[log.related_entity_id || ''] || log.related_entity_id}`;
      case 'Rejection': return `Rejected: ${mockEntityNames[log.related_entity_id || ''] || log.related_entity_id}`;
      case 'AI_Processing': return `Processed: ${mockEntityNames[log.related_entity_id || ''] || log.related_entity_id}`;
      case 'Create': return `Created: ${log.event_context.product_name}`;
      case 'View': return `Viewed by public user`;
      default:
        const keys = Object.keys(log.event_context);
        return keys.length > 0 ? `${keys[0]}: ${log.event_context[keys[0]]}` : 'No context';
    }
  };
  ...
  return (
    <Tabs defaultValue="events" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="changes">Product Change Log</TabsTrigger>
        </TabsList>
        <TabsContent value="events">
            <Card>
                <CardHeader>
                <CardTitle className="font-headline">Event Log</CardTitle>
                <CardDescription>Chronological record of all major system and user events.</CardDescription>
                </CardHeader>
                <CardContent>
                <Table>
                    <TableHeader>
                            ...
                            <TableRow key={log.log_id}>
                                <TableCell>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge variant={config.badge}>{log.event_type.replace('_', ' ')}</Badge>
                                    </div>
                                </TableCell>
                                <TableCell>
                                    <div className="flex flex-col">
                                        <span className="font-medium">{mockEntityNames[log.product_id] || log.product_id}</span>
                                        <span className="text-xs text-muted-foreground">{getEventContextSummary(log)}</span>
                                    </div>
                                </TableCell>
                                <TableCell>
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          {selectedEventLog && (
            <div className="grid gap-6 py-4 text-sm max-h-[70vh] overflow-y-auto pr-4">
              ...
              {selectedEventLog.geo_info && (
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                   <div><span className="font-medium text-muted-foreground block">IP Address:</span> {selectedEventLog.geo_info.ip}</div>
                   <div><span className="font-medium text-muted-foreground block">Country:</span> {selectedEventLog.geo_info.country}</div>
                </div>
              )}
